...
\begin{proof}
	Soit $\mathcal{P}$ un système de réprésentants des irréductiobles de $A$.
	On écrit
...
\begin{proof}
	...
	\end{eqnarray*}
	On a donc que le coéfficient de degré $i_0 + j_0$ de $PQ$ est:
	\begin{eqnarray*}
	...
	Donc le coéfficient de degré $i_0+j_0$ de $PQ$ n'est pas divisible par $p$ ce qui contredit les hypotèses.
...
\begin{remarque}
	$A, K  = Frac(A)$ Alors il y a un plogement $\phi: A \to K$ et en général, on identifie $A$ avec $\phi(A)$ de sorte que $A \subset K$.
	Ainsi un polynôme $P \in A[X]$  peut etre vu comme un polynôme dans $K[X]$.
\end{remarque}


\begin{theorem}[Clasification des irréductibles]
...
\begin{proof}
	...
	Si $P$ primitif avec $\deg \geq 1$ dans $A[X]$, qui est irréductible dans $K[X]$, on écrit
	$P = QR$, avec $Q,R \in A[X]$.

	On a que $c(P) = c(Q) c(R) \in A^\times$ donc $c(Q) \in A^\times$ et $c(R) \in A^\times$.
	La relation $P = QR \in K[X]$ implique que $Q$ et $R$ sont de dégré 0 (car $P$ primitif).
	Comme ils sont primitifs, $Q$ ou $R \in A^\times \implies \in A[X]^\times$
	...
	Montrons $P$ irréductible dans $K[X]$.
