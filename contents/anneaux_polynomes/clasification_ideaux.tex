...
\begin{theorem}
	Soit $A$ un anneau principal. Alors les udéaux premiers non nuls de $A[X]$ sont les idéaux de la liste suivante:
	\begin{itemize}
		\item $\goatp_pi = <\pi>$ ou $\pi$ est un élément irréductible de $A$. %TODO: Add { saying that they are the irreducible elements of A[X]}
	...
	\begin{itemize}
		...
		\item Si $A$ possede une infinitée d'éléments irréductibles deux à deux disjoints non associés, alors $p_f$ est premier mais pas maximal.
		\item $\goatm_{\pi,f}$ est maximal.
	\end{itemize}
\end{theorem}


%TODO: Format this properly on a "rappel" environment and add more info
\begin{itemize}
	\item Comme $\pi$ est irréductible on a que $A<\pi>$ est un corpps, donc $\goatp_{\pi}$ n'est pas maximal. %TODO: Check this
	\item Soit $f \in A[x]$, $\bar{f}$ est la classe de $f$ dans $A/<\pi>[X] = A[X]/<\pi>$.
	      $f$ irréductible modulo $\pi$ signifie que $\bar{F}$ est irréductible dans $A/<\pi>$.
\end{itemize}
