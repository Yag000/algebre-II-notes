...
\begin{proof}
	...
	$\Leftarrow$ Si $A$ est intègre, alors pour tous $P, Q \in A[X]$, on a $\deg(P Q)=\deg(P)+\deg(Q)$.
	Puisque l'anneau $A$ est commutatif et non trivial (le cas trivial n'étant pas intègre), il en est de même de $A[X]$.
	Soient $P, Q \in A[X]$ non nuls. on a donc $\deg(P) \geqslant 0$ et $\deg(Q) \geqslant 0$.
	Mais alors, on a :
	$$
		\deg(P Q)=\deg(P)+\deg(Q) \geqslant 0,
	$$
