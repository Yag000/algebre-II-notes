...
\begin{proof}
    ...
    2) $\implies$ 1):\\
    Soit $\mathrm{P}$ un système de représentant des indestructibles.
    On a alors $u \cdot \prod_{p \in \mathrm{P}} p^{n_q}$ (divisible par $q^{n_q} \in <q>$ ) $= v \cdot \prod_{p \in \mathrm{P}} p^{m_q}$ divisible par $q^{m_q} \in <q>$)
    Si il existe $q \in \mathbb{P}$ tel que $m_q > n_q$, alors $q$ divise $u \cdot \prod_{p \ne q} p^{n_q}$ ($\in <q>$), ce qui n'est pas possible par 2).
...
\begin{proof}
    ...
    1) $\implies$ 3):\\
    $A$ est factoriel. Si $a$  divise $x$, on écrit $a$, $b$, $c$ sous la forme $u \cdot \prod_{p \in \mathrm{P}} p^{v_{p(x)}}$.
    On a alors $\forall p \in \mathrm{P}$, $v_p(a) \leqslant v_p(b) \leqslant v_p(c)$ car $a$ divise $bc$.
    Si $v_p(a) \geqslant b$, alors $v_p(b) = 0$.
    Pour $H_p \in \mathrm{P}$, $v_p(a) \leqslant v_p(c)$, donc $a$ divise $c$, qui vérifie 3).
